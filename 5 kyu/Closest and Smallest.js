...
function challenge(string) {
  ...
  let weightArr = string
    .match(/\d+/g)
    .map((num, index) =>{
      const weight = num.split('').reduce((acc, val) => acc + parseInt(val), 0)
      return [weight, index, parseInt(num)];
    })

  weightArr = sort(weightArr);
  
  return weightArr.reduce(({closestPair, latest}, num) => {
      if(closestPair.length <= 1) return { closestPair: [...closestPair, num], latest: num };
      
      const currentDiff = Math.abs(latest[0] - num[0]);
      const closestPairDiff = Math.abs(closestPair[0][0] - closestPair[1][0]);
      
      if (currentDiff < closestPairDiff) {
        if (currentDiff === 0) return { closestPair: [latest, num].sort((a, b) => a[1] - b[1]), latest: num };
        return { closestPair: [latest, num], latest: num };
      }

      return { closestPair, latest: num };
    }, { closestPair: [], latest: [] })
    .closestPair;
}
